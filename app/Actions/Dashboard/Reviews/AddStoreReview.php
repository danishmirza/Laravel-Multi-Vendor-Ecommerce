...
class AddStoreReview
{
    ...
    public function handle($ratingId, $rating, $comment)
    {
        try {
            StoreReview::where(['id' => $ratingId])->update([
                'rating' => $rating,
                'comment' => $comment,
                'is_given' => 1
            ]);
    ...
    public function rules(){
        ...
        return [
            'review_id' => [
                'required',
                Rule::exists('store_reviews', 'id')->where('user_id', auth()->id())->where( 'is_given' , 0)
            ],
    ...
    {
        $router->post('dashboard/store-reviews/add', static::class)->name('dashboard.store-reviews.add');
    }
